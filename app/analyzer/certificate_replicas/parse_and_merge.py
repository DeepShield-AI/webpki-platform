...
'''
    Second step, read all top-1m domain separate files, and merge them into
    several large files, with leaf certificate parsed

    This heavily lower the burden of PC file system
'''
...
class ReplicaCounting():
            ...
            with open(file_path, "r") as file:
                ...
                for json_str in self.split_json_objects(data):
                    try:
                        ...
                        leaf_cert_native = pem_parser.parse_native(entry['leaf'])
                        leaf_cert_native['timestamp'] = entry['timestamp']
                        self.total_dict[fqdn].append(leaf_cert_native)
    ...
    def start(self):
        with Progress(
            ...
            self.total = len(file_name_list)

            # 30000 is a good chunk size for max_worker = 10
            split = 30000
            chunk = math.ceil(self.total / split)

            self.progress_task = self.progress.add_task("[Waiting]", total=self.total)
            for i in range(2, chunk):
                # Note: due to file descriptor and other resource constraints,
                # the number of max_workers cannot be too large, typically 10-20
                with ThreadPoolExecutor(max_workers=10) as executor:

                    for file_name in file_name_list[i*split : (i+1)*split]:
                        executor.submit(self.scan_thread, file_name)
                        # executor.submit(self.scan_thread, file_name).result()

                    executor.shutdown(wait=True)
                    print(f"All threads for file id {i*split} to {(i+1)*split - 1} finished.")

                    with open(os.path.join(self.save_dir, f'total_out_50M_{i}.json'), 'w') as f:
                        json.dump(self.total_dict, f, indent=4, default=custom_serializer)

                    # Sleep here cuz the need for computer file resources to be freed completely
                    sleep(10)
                    self.total_dict = {}
